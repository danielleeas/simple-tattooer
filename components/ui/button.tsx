    ...
    variants: {
      variant: {
        default: cn(
          ...
          Platform.select({ web: 'hover:bg-primary/60' })
        ),
        destructive: cn(
        ...
        outline: cn(
          'border border-border-white transparent shadow-sm shadow-black/5 active:bg-accent dark:bg-input/30 dark:active:bg-input/50',
          Platform.select({
      ...
      size: {
        ...
        lg: cn('h-12 px-6 sm:h-10', Platform.select({ web: 'has-[>svg]:px-4' })),
...
function Button({ className, variant, size, ...props }: ButtonProps) {
  return (
    <TextClassContext.Provider value={buttonTextVariants({ variant, size, disabled: props.disabled })}>
      <Pressable
        className={cn(
          buttonVariants({ variant, size }), 
          props.disabled && 'opacity-50 bg-disabled border-0 cursor-not-allowed',
          Platform.select({
            web: props.disabled && 'pointer-events-none select-none'
          }),
          className
        )}
        role="button"
