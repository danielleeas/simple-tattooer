...
import { decodeTemp } from '@/lib/utils';
...
export function ClientSwitcherModal({ visible, onClose }: ClientSwitcherModalProps) {
  ...
  const handleArtistSwitch = async (email: string, temp: string) => {
    if (!email || !temp) return;
    try {
      setSwitching(true);
      const password = decodeTemp(temp);
        ...
        {!loading && (
          <View>
            <ScrollView showsVerticalScrollIndicator={false}>
              {/* Current Account */}
              <Pressable onPress={() => handleArtistSwitch(artist?.email || '', artist?.temp || '')} className="mb-4 px-4 py-2 bg-background rounded-xl border border-border">
              ...
              <View>
                ...
                {accounts.map((account) => (
                  <Pressable onPress={() => handleSwitchAccount(account.email)} key={account.id}
                    className="mb-4 px-4 py-2 bg-background rounded-xl border"
                    style={{ borderColor: currentEmail === account.email ? '#10B981' : '#1E293B', borderWidth: 1 }}>
                    <View className="flex-row items-center gap-3">
                      <View className="w-12 h-12 rounded-full bg-border items-center justify-center">
                        <UserCircle size={32} color="#888" />
                      </View>
