...
export default function EditSpotConventionPage() {
    ...
    const loadSpotConvention = useCallback(async () => {
        ...
        try {
            ...
            if (res.success && res.data) {
                ...
                toast({ variant: 'error', title: res.error || 'Failed to load spot convention' });
            }
        } catch (e) {
            toast({ variant: 'error', title: 'Failed to load spot convention' });
        } finally {
            setFetching(false);
        }
    }, [id]);
    ...
    const handleSave = async () => {
        ...
        try {
            setLoading(true);
            const result = await updateSpotConvention(String(id), {
